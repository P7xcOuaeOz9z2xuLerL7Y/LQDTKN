...
from questions import validation_format, BASE_URL, scan_format, proof_gate_format
    ...
    def ask_proof_gate(self, filename, question_gotten):
        ...
        for _ in range(10):
            try:
                ...
                textarea.click()
                textarea.clear()
                formatted_question = proof_gate_format(question_gotten)
...
class GetValidatedReports:
    ...
    def get_report(self, url):
        ...
        try:
            ...
            if saved_path:
                print(f"Saved DeepWiki validation candidate for question {url} to {saved_path}")
            else:
                print(f"No validation candidate saved for: '{url}'")
...
def generate_scanned_questions_for_ask():
    ...
    # Scanner inputs can be legacy Markdown reports or structured JSON reports.
    audited_files = sorted(
        list(Path(scanned_directory).glob('*.md'))
        + list(Path(scanned_directory).glob('*.json'))
    )
...
def generate_proof_gate_questions_for_ask():
    input_dirs = [
    ...
    proof_gate_pending_directory = os.environ.get("PROOF_GATE_PENDING_DIR", "proof_gate_pending")
    validated_questions_directory = os.environ.get("VALIDATED_QUESTIONS_DIR", "validated_questions")

    os.makedirs(proof_gate_pending_directory, exist_ok=True)
    os.makedirs(validated_questions_directory, exist_ok=True)

    candidate_files = []
    for directory in input_dirs:
        candidate_files.extend(directory.glob("*.md"))
    ...
    for file_path in candidate_files:
        try:
            ...
            dest_path = os.path.join(proof_gate_pending_directory, file_path.name)

            if os.path.exists(dest_path):
                base_name = file_path.stem
                extension = file_path.suffix
                timestamp = int(time.time())
                dest_path = os.path.join(proof_gate_pending_directory, f"{base_name}_{timestamp}{extension}")
    ...
    print(f"Successfully moved {len(moved_files)} files to {proof_gate_pending_directory}")
    return moved_files
